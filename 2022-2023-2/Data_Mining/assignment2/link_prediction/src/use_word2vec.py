...
class Node2Vec:
    ...
    def train(self, embed_size=128, epochs=50, neg_spl=10, load_model=None):
        ...
        for _ in range(self._num_walks):
            ...
            for node in nodes:
                corpus.append(self.single_random_walk(node))

        print("Node2Vec: Biased Walk Dataset ready.")
        print()

        corpus = [list(map(str, path)) for path in corpus]
        print("Node2Vec: Training Word2Vec...")
        self.model = Word2Vec(corpus, vector_size=embed_size, window=self._half_window_size, min_count=0, sg=1, workers=8, epochs=epochs, negative=neg_spl)
        print("Node2Vec: Word2Vec training finished.")
        print()
    ...
    def get_embeddings(self, ):
        for node in self.graph.nodes():
            emb = self.model.wv[str(node)].copy()
            self._embeddings[node] = torch.FloatTensor(emb)
    ...
    def train_classifier(self, epochs=150, neg_spl=10, load_pairs=None, load_classifier=None):
        ...
        if load_pairs is not None:
            ...
            # positive samples
            for edge in self.graph.edges():
                pairs.append((edge[0], edge[1], 1))
                pairs.append((edge[1], edge[0], 1))
            ...
            for node in self.graph.nodes():
                for _ in range(neg_spl):
                    ...
                    while neg in self.graph.neighbors(node):
                        neg = np.random.choice(len(self.graph.nodes()))
                    pairs.append((node, self._idx2node(neg), 0))
    ...
    def predict(self, source, target):
        ...
        prob = self._classifier(torch.cat([torch.tensor(enc1), torch.tensor(enc2)], dim=0)).detach().numpy()[1]
        # prob = self._classifier(enc1 - enc2).detach().numpy()[1]
...
def evaluate():
    pred = {}
    with open('../data/predictions.csv', 'r') as test:
        test.readline()
        csv_reader = csv.reader(test)
        for row in csv_reader:
            pred[int(row[0])] = float(row[1])
    
    correct = 0.
    total = 0.
    with open('../data/lab2_truth.csv', 'r') as truth:
        truth.readline()
        csv_reader = csv.reader(truth)
        for row in csv_reader:
            total += 1
            correct += pred[int(row[0])]
