...
for x in train dev test; do
  steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 4 data/$x exp/make_mfcc/$x $mfccdir || exit 1;
...
steps/decode.sh --cmd "$decode_cmd" --nj 4 --config conf/decode.config \
...
steps/decode_fmllr.sh --cmd "$decode_cmd" --nj 4 --config conf/decode.config \
...
echo "Tri5a - internal checkpoint";
