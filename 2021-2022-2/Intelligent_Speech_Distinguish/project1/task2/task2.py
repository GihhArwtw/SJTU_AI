...
def data_prep(in_data):
    
    """
    
        Prepare the data for network training on sklearn.
    
    """
    
    in_dataset = DataLoader(in_data, batch_size=1, shuffle=True)
    
    dataset = []
    labels = []
    for batch, (data, target) in enumerate(in_dataset):
        data = data[0].tolist()
...
mfcc, labels = data_prep(train_mfcc)
mfcc_mlp = MLPClassifier(hidden_layer_sizes = (24, 12, 6, 4, 2), max_iter=Training_epoch, shuffle=False).fit(mfcc, labels)

joblib.dump(mfcc_mlp, 'DNN_mfcc_best_on_sklearn.pkl')



mfcc_mlp = joblib.load('DNN_mfcc_best_on_sklearn.pkl')

valid_mfcc, dev_labels = data_prep(dev_mfcc)
out_mfcc = mfcc_mlp.predict_proba(valid_mfcc)

pred_mfcc = np.append(out_mfcc[:-1,1],out_mfcc[-1,1])
print(get_metrics(pred_mfcc,dev_labels))
print(1. - np.abs(np.array(pred_mfcc>=0.5)-np.array(dev_labels)).mean())



fbanks, labels = data_prep(train_fbank)
fbank_mlp = MLPClassifier(hidden_layer_sizes = (40, 20, 10, 4, 2), max_iter=Training_epoch, shuffle=False).fit(fbanks, labels)
joblib.dump(fbank_mlp, 'DNN_fbank_best_on_sklearn.pkl')



fbank_mlp = joblib.load('DNN_fbank_best_on_sklearn.pkl')

valid_fbank, dev_labels = data_prep(dev_fbank)
out_fbank = fbank_mlp.predict_proba(valid_fbank)

pred_fbank = np.append(out_fbank[:-1,1],out_fbank[-1,1])
print(get_metrics(np.append(pred_fbank),dev_labels))
print(1. - np.abs(np.array(pred_fbank>=0.5)-np.array(dev_labels)).mean())
...
'''
    Since the two network trained on pytorch based on our handcrafted model does not perform excellently on dev set,
        we only use the two network trained on sklearn to predict on the test set.
'''

import os
files= os.listdir("../wavs/test")

fbank_file = open("test_label_task.txt","w")
mfcc_file = open("test_label_task_mfcc.txt","w")
for file in files:
    frames, fs = Frame_Seg(f"../wavs/test/{file}", window=Windowing, emph=Emph, emph_alpha=alpha)

    spec = stft_power(frames, nfft=nfft)
    fbank, mfcc = mel_filter(spec, fs, nfft=nfft)
    
    out_mfcc = mfcc_mlp.predict_proba(mfcc)
    pred_mfcc = np.append(out_mfcc[:-1,1],out_mfcc[-1,1])
    pred_mfcc = (pred_mfcc>=0.5)
    
    print(pred_mfcc.mean())
    
    out_fbank = fbank_mlp.predict_proba(fbank)
    pred_fbank = np.append(out_fbank[:-1,1],out_fbank[-1,1])
    pred_fbank = (pred_fbank>=0.5)
    
    print(pred_fbank.mean())
    
    # Write the label into the output file.
    fbank_file.write(file[:-4])
    fbank_file.write(" ")
    fbank_file.write(prediction_to_vad_label(pred_fbank))
    fbank_file.write("\n")
    
    mfcc_file.write(file[:-4])
    mfcc_file.write(" ")
    mfcc_file.write(prediction_to_vad_label(pred_mfcc))
    mfcc_file.write("\n")
