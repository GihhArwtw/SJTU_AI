...
mfcc_mlp = MLPClassifier(hidden_layer_sizes = (24, 12, 6, 4, 2), max_iter=1000, shuffle=False).fit(mfcc, labels)
...
print(get_metrics(np.append(out_mfcc[:-1,1],out_mfcc[-1,1]),dev_labels))


# DNN on Fbank.

fbanks, labels = data_prep(train_fbank)
fbank_mlp = MLPClassifier(hidden_layer_sizes = (40, 20, 10, 4, 2), max_iter=1000, shuffle=False).fit(fbanks, labels)

valid_fbank, dev_labels = data_prep(dev_fbank)
out_fbank = fbank_mlp.predict_proba(valid_fbank)

joblib.dump(mfcc_mlp, 'DNN_fbank_best_on_sklearn.pkl')
print(get_metrics(np.append(out_fbank[:-1,1],out_fbank[-1,1]),dev_labels))
