...
template <typename eT>
void BTree<eT>::LevelOrder() const
{
	Queue<node<eT>*> que;
	node<eT>* p;
...
void BTree<eT>::MidOrder() const
{
	...
	sta.push(p);
	while (p->left) { sta.push(p->left); p = p->left; }//sta.push(p=p->left);
	while (!sta.empty())
	{
		p = sta.pop();
		(std::cout) << p->datum;
		if (p->right)
		{
			p = p->right; sta.push(p); //sta.push(p=p->right);
			while (p->left) { sta.push(p->left); p = p->left;	} //sta.push(p=p->left);
		}
	} 
	...
	while (!sta.empty())
	{
		...
		{
			stn.push(1);
			if (p->left) { sta.push(p->left); stn.push(0); }
			continue;
		}
		(std::cout) << p->datum;
		sta.pop();
		if (p->right) { sta.push(p->right); stn.push(0); }
	}
	/* ====== end of choice ====== */
...
void BTree<eT>::PostOrder() const
{
	...
	/* ====== while version 01 ====== */
	//with a little adaption, it could be used to find all x's ancestors.
	if (!root) return;
	Stack<node<eT>*> sta;
	Stack<int> stn;
	node<eT>* p;
